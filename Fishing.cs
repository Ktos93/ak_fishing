...
namespace ak_fishing
{
    public class Fishing : BaseScript
    {
        ...
        public Fishing()
        {
            ...
            EventHandlers["ak_fishing:UseBait"] += new Action<string>(UseBait);
            ...
            Function.Call((Hash)0x9CB1A1623062F402,  sellBlip, "Market Rybny");
            Tick += FishingCore;
        ...
        private async Task FishingCore()
        {
            ...
            if (API.Vdist(coords.X, coords.Y, coords.Z , 2998.792f, 477.8959f, 42.0f) <2f && !this.Selling)
            {
                if (!this.SellActive)
                {
                    ...
                    long str = Function.Call<long>((Hash)0xFA925AC00EB830B9, 10, "LITERAL_STRING", "Naciśnij ~INPUT_JUMP~ aby sprzedać ryby");
                ...
                if (API.IsControlJustPressed(0, 0xD9D0E1C0))
                {
                    ...
                    Exports["progressBars"].startUI(2000, "Sprzedaż w trakcie...");
                    TriggerServerEvent("ak_fishing:Sell");
        ...
        private async Task FishingMinigame()
        {
            ...
            if (this.FishingState_ == 6)
            {
                    ...
                    if (this.TargetFish != 0)
                {
                    ...
                    while (API.Vdist(bobberCoords.X, bobberCoords.Y, bobberCoords.Z, fishCoords.X, fishCoords.Y, fishCoords.Z) > 1.0f && this.FishingState_ == 6)
                    {
                    ...
                    while (API.Vdist(bobberCoords.X, bobberCoords.Y, bobberCoords.Z, fishCoords.X, fishCoords.Y, fishCoords.Z) < 2.2f && !this.Hooked && this.FishingState_ == 6)
                    {
                        await BaseScript.Delay(10);
                        fishCoords = API.GetEntityCoords(this.TargetFish, true, true);
                        bobberCoords = API.GetEntityCoords(data.BobberHandle, true, true);
        ...
        internal unsafe struct UnsafeFishTaskState
        {
            ...
            public FishTaskState GetData()
            {
                return new FishTaskState(fishingState, throwingTargetDistance, distance, curvature, unk0, hookFlag,
                    transitionFlag, fishHandle, fishweight, fishPower, scriptTimer, hookHandle, bobberHandle, rodShakeMult,
                    unk1, unk2, unk3, shakeFightMult, fishSizeIndex, fishingRodX, fishingRodY, tension);
            }
        }
        ...
        public struct FishTaskState
        {
            ...
            public FishTaskState(int fishingState, float throwingTargetDistance, float distance, float curvature, float unk0, int hookFlag,
                 int transitionFlag, int fishHandle, float fishweight, float fishPower, int scriptTimer, int hookHandle, int bobberHandle,
                 float rodShakeMult, float unk1, float unk2, int unk3, float shakeFightMult, int fishSizeIndex, float fishingRodX, float fishingRodY, float tension)
            {
        ...
        private void UseBait(string bait)
        {
            ...
            if (data.FishingState  == 1) {
                ...
                TriggerServerEvent("ak_fishing:RemoveBait", baitName);
            }
        ...
        private async Task PlayPtfx()
        {
            ...
            Function.Call(Hash.START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD, "scr_mg_fish_struggle",
                fishCoords.X, fishCoords.Y, fishCoords.Z, 0f, 0f, (float)(rnd.NextDouble() * 360.0), 1f, 0, 0, 0);
        ...
        {
            string str = "Zaczep rybe na haczyk";
            var prompt = Function.Call<int>((Hash)0x04F97DE45A519419);
        ...
        public int GetClosestFish(int bobberHandle)
        {
            ...
            int size = Function.Call<int>((Hash)0x59B57C4B06531E1E, bobberCoords.X, bobberCoords.Y, bobberCoords.Z, 10f, itemSet, 1);
            int output = 0;
            if (size == 0) { return 0; }
